...
events[0].write('CMTSOLUTION', format='CMTSOLUTION')
...
with open('receiver_new.dat', 'w') as fid:
...
ax.plot(event.longitude, event.latitude, '*', color='grey',
...
for stat in stats_sort:
    ax.plot(stat.longitude, stat.latitude, 'v', color='grey',
            transform=ccrs.PlateCarree(), markersize=10)
ax.legend(fontsize=12, numpoints=1, markerscale=2.0)

fname = './stations.pdf'
fig.savefig(filename=fname, bbox_inches='tight',
            dpi=300)
#
#
