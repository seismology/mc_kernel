...
!========================
  ...
  public :: init_time_window_misfit_medium
  public :: allocate_arrays
  public :: misfit_kernels
  public :: waveform_kernels
...
! end time series analysis ----------------------------------------------------
  ...
  do i = 1,ntw
     call compute_denominator(t1(i), t2(i), time, denomu(i), denomv(i))
  enddo
...
!----------------------------------------------------------------------------------------------------------
! These variables may just as well be local.
...
!-------------------------------------------------------------------------
  subroutine compute_denominator(t1,t2,time,denomu1,denomv1)
  ...
  include 'mesh_params_kernel.h'
    ...
    real(kind=realkind), dimension(ndumps), intent(in) :: time
    integer :: it
    ...
    do it=1, ndumps
       if (time(it)>=t1 .and. time(it)<=t2) then
          if (src_type=='monopole') then 
             denomv1 = denomv1 + v0(it,1)**2 + v0(it,2)**2
             denomu1 = denomu1 + u0(it,1)**2 + u0(it,2)**2
          else
             denomv1 = denomv1 + v0(it,1)**2 + v0(it,2)**2 + v0(it,3)**2
             denomu1 = denomu1 + u0(it,1)**2 + u0(it,2)**2 + u0(it,3)**2
          endif
       endif
    enddo
...
subroutine read_disp_velocity_seis(thr,phr)
    ...
    !use nc_routines, only: nc_read_surf, nc_read_surfcoords
    use data_mesh, only: use_netcdf
    use mpi
    implicit none
    include 'mesh_params.h'
    include 'mesh_params_kernel.h'
...
! find closest receiver location for thr
    ...
    if (use_netcdf) then
        !call nc_read_surfcoords(nsurf, surftheta)
    else 
