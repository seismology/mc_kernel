...
subroutine frequency_domain_waveform_kernels
  ...
  use data_arrays, only: time_mean, kerspec, kerphys
...
subroutine wavefields_time2frequency_lam
  ...
  use nc_routines, only : nc_read_summedstraintrace
  use mpi
  real(kind=realkind), dimension(npts) :: usrc, urec
  ...
  do isim=1, nsim
      ...
      write(6,*) mynum, 'azimuthal fwd prefactor 1 min/max:', minval(azim1_fwd), &
                 maxval(azim1_fwd)
      if (use_netcdf) then
          !call nc_read_summedstraintrace(kerphys, azim1_fwd, 'src')
      else
          do it=1, ndumps
              usrc = 0.
              ...
              if (save_snaps .and. mod(it,20)==0) &
                  call save_kernel(npts, usrc, fname, appidump1)

              kerphys(it+1,:) = kerphys(it+1,:) + usrc
          enddo
      end if
  enddo
  ...
  if (use_netcdf) then
      !call nc_read_summedstraintrace(kerphys, azim1_bwd, 'rec')
  else
      do it=1, ndumps
         urec = 0.
         if (mod(it,ndumps/10) == 0 .and. mynum == 0) &
         ...
         if (save_snaps .and. mod(it,20) == 0) &
             call save_kernel(npts, urec, fname, appidump1) 
         
         kerphys(it+1,:) = azim1_bwd * urec
      enddo
  end if
...
subroutine wavefields_time2frequency_rho
  ...
  real(kind=realkind), dimension(npts) :: vsrc, vrec
  real(kind=realkind), dimension(npts,ndim) :: vrot
