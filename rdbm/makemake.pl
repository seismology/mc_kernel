...
if ($ARGV[0] eq 'ifort'){
    if ($ARGV[1] eq 'debug'){
	$F90_strg = 'ifort  -vec-report:0 -g -O2 -shared-intel  -mcmodel=medium -ftz -check all -check noarg_temp_created -debug  -check -traceback';
    } else {
	$F90_strg = 'ifort  -vec-report:0 -g -O4 -xHOST -shared-intel'; 
    }
}
elsif ($ARGV[0] eq '-h'){
	...
	$F90_strg = 'gfortran -Warray-temporaries -fcheck-array-temporaries -fbounds-check -frange-check -pedantic -fbacktrace';
    } else {
	$F90_strg = 'gfortran -O3  -fbacktrace -g';
    }
}
###################################################################################
