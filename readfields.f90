...
module readfields
    ...
    type meshtype
        ...
        real(kind=sp), allocatable         :: lambda(:), mu(:)      ! 
        real(kind=sp), allocatable         :: phi(:), xi(:), eta(:) ! 
...
subroutine read_meshes(this)
    ...
    if (trim(this%dump_type) == 'displ_only') then
        ...
        call nc_getvar_by_name(ncid   = this%fwd(1)%mesh,  &
                               name   = 'mesh_S',          &
        ...
        call nc_getvar_by_name(ncid   = this%fwd(1)%mesh,  &
                               name   = 'mesh_vp',         &
                               values = this%fwdmesh%vp  )
                    
        call nc_getvar_by_name(ncid   = this%fwd(1)%mesh,  &
                               name   = 'mesh_vs',         &
                               values = this%fwdmesh%vs  )
                    
        call nc_getvar_by_name(ncid   = this%fwd(1)%mesh,  &
                               name   = 'mesh_rho',        &
                               values = this%fwdmesh%rho )
                    
        call nc_getvar_by_name(ncid   = this%fwd(1)%mesh,  &
                               name   = 'mesh_lambda',     &
                               values = this%fwdmesh%lambda)
                    
        call nc_getvar_by_name(ncid   = this%fwd(1)%mesh,  &
                               name   = 'mesh_mu',         &
                               values = this%fwdmesh%mu  )
